...
import pytest
from jmd import JMDError, JMDErrorItem, is_error_document, parse_error
...
class TestIsErrorDocument:
    ...
    def test_data_document(self):
        assert is_error_document("# Order\nid: 42") is False

    def test_error_with_leading_blank(self):
        assert is_error_document("\n# Error\nstatus: 404") is True

    def test_non_error_heading(self):
...
class TestErrorParsing:
    ...
    def test_extra_fields_in_extra(self):
        err = parse_error("# Error\nstatus: 500\ncode: internal\nrequest_id: abc123")
        assert err.extra.get("request_id") == "abc123"

    def test_non_error_document_raises(self):
